...
<body>
	<article class="page-container">
		<form id="form" action="/admin/${pageDir}/update" method="post" class="form form-horizontal">
			<input type="hidden" id="${firstName}" name="${firstName}" th:value="${r"${"}${smallClassName}.${firstName}${r"}"}" />
			<#list list as item>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-8">
					...
					<textarea class="textarea" id="remark" name="remark" placeholder="备注" maxlength="200" th:text="${r"${"}${smallClassName}.${item.beanLable}${r"}"}"></textarea>
					<p class="textarea-numberbar"><em class="textarea-length">0</em>/200</p>
					</#if>
					<#if item.beanLable != "remark">
					<input type="text" class="input-text" id="${item.beanLable}" name="${item.beanLable}" placeholder="${item.beanLable}" th:value="${r"${"}${smallClassName}.${item.beanLable}${r"}"}" />
					</#if>
				</div>
			</div>
			</#if>
